...
const cards = [
  {
    ...
    description: 'We pioneer next-generation semiconductor technologies that redefine what\'s possible in analog and digital signal processing.',
    longDescription: 'Our R&D team constantly explores new frontiers in chip design, leveraging AI and quantum-inspired algorithms to create solutions that are 10x more efficient than conventional approaches.',
    icon: Zap,
    image: 'https://www.sportsbusinessjournal.com/resizer/v2/JAM7U3LGC5AA7LMIQ73XHFYSWY.JPG?auth=f8e4b573202522fc4a6537a170b75e39d5a0d0689a850132555d5cd531d68e2e&width=800&height=533',
  ...
  },
  {
    ...
    description: 'Every chip undergoes rigorous testing and validation to ensure highest reliability and performance standards.',
    longDescription: 'Our ISO 9001:2024 certified facilities maintain strict quality control measures, with automated testing at every stage of production.',
    icon: Shield,
  ...
  },
  {
    ...
    description: 'Serving customers across 50+ countries with local support and rapid deployment capabilities.',
    longDescription: 'Our global network of design centers and sales offices ensures we\'re always close to our customers, providing real-time support and collaboration.',
    icon: Globe,
  ...
  },
  {
    ...
    description: 'Committed to eco-friendly manufacturing and energy-efficient semiconductor solutions.',
    longDescription: 'We\'ve reduced our carbon footprint by 40% through innovative manufacturing processes and power-efficient chip designs.',
    icon: Target,
        ...
        {cards.map((card, index) => {
          ...
          return (
            <div
              ...
              className="relative min-h-screen w-full bg-white/30"
            >
              <div className="max-w-6xl mx-auto px-4 md:px-6 py-16 w-full min-h-screen flex items-center">
                <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center w-full ${
                  ...
                  {/* Image Section */}
                  <div className={`relative ${card.layout === 'right' ? 'lg:order-2' : 'lg:order-1'}`}>
                    <div className="card-image relative rounded-2xl overflow-hidden shadow-xl">
                      <div className="relative aspect-[4/3]">
                        ...
                        />
                        <div className={`absolute inset-0 bg-gradient-to-r ${card.gradient} mix-blend-multiply opacity-20`} />
                      </div>
                      
                      {/* Floating Badge */}
                      <div className="absolute top-4 right-4 px-3 py-1.5 rounded-lg bg-white shadow-lg z-10">
                        <span className="text-xs font-bold text-slate-800">{card.stats}</span>
                      </div>

                      {/* Decorative border */}
                      <div className={`absolute inset-0 rounded-2xl border-2 border-${card.color}-200 opacity-40 pointer-events-none`} />
                    </div>
                  </div>

                  {/* Content Section */}
                  <div className={`${card.layout === 'right' ? 'lg:order-1' : 'lg:order-2'}`}>
                    <div className="space-y-5">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="px-3 py-1 rounded-full bg-slate-100 border border-slate-200">
                          <span className="text-xs font-semibold text-slate-600">{card.subtitle}</span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-3">
                        <div className="card-stat p-3 rounded-xl bg-white border border-slate-200 shadow-sm transition-all duration-300 hover:shadow-md hover:scale-105">
                          <p className="text-xl md:text-2xl font-bold text-slate-900">{card.stats}</p>
                          <p className="text-xs text-slate-500">Industry Recognition</p>
                        </div>
                        <div className="card-stat p-3 rounded-xl bg-white border border-slate-200 shadow-sm transition-all duration-300 hover:shadow-md hover:scale-105">
                          <p className="text-xl md:text-2xl font-bold text-slate-900">{card.metric}</p>
